...
::: tip Android 端差异
Android 版的插件运行时由**内嵌 Node.js Mobile** 承载（与在线 API、歌词获取共用同一运行时），沙箱环境、`splayer` API 与错误码与桌面版一致；进程模型与调试方式存在差异——Android 端没有桌面版独立的插件 host 子进程，调试请通过 WebView 远程调试或 logcat 查看内嵌运行时日志，而非桌面 DevTools。
:::
